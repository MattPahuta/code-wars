...
function baublesOnTree(baubles, branches) {
  ...
  const babulesOnBranch = Math.floor(baubles / branches);
  const remainder = baubles % branches;

  return new Array(remainder).fill(babulesOnBranch + 1)
    .concat(new Array(branches - remainder).fill(babulesOnBranch));
